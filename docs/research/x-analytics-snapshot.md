# X analytics snapshot schema

This note defines the contract-first fake X analytics snapshot for issue #19.

## Quick path

1. Use `examples/x-analytics-snapshot.fake.yaml` as the fake snapshot source.
2. Run `scripts/validate-x-analytics-snapshot.sh`.
...
Issue #17 established that the reviewed public Buffer API docs do not expose read-only X analytics metrics.
...
| Field group | Purpose |
|---|---|
| `source` | States that the snapshot comes from a fake manual-export placeholder source and that `live_buffer_api: false`. |
| `snapshot_window` / `captured_at` | Defines when the snapshot claims to summarize performance. |
| `namespace_keys` | Anchors the snapshot to `content-ledger`, `network/x`, and `strategy` namespaces. |
| `content_metrics` | Lists one normalized record per X content item. |
...
3. Each item also keeps `planning_reference_id` so analytics can be traced back to either existing ledger history or the fake X queue plan when needed.

This gives later ingestion work a stable join surface without pretending that live X analytics are already available.

## Validation

```bash
git diff --check
bash -n scripts/validate-x-analytics-snapshot.sh
scripts/validate-x-analytics-snapshot.sh
npx --yes yaml-lint examples/x-analytics-snapshot.fake.yaml
