...
5. Install the external OpenClaw Discord plugin before live validation.
6. Name routed channels with `<network-slug>-<project-slug>`.
7. Keep real guild and channel IDs outside the repo.
8. Route channel-local memory by raw Discord IDs.
9. Route durable project reads by approved project/network slugs.
10. Ask for human approval before durable project writes.
11. Load `skills/discord-approval-gate/SKILL.md` for Discord write-like intents before any persistence.
12. Use `docs/operations/discord-approval-responses.md` for approval prompts and audit trail requirements.
13. Use `docs/architecture/channel-context-namespace-mapping.md` and `examples/discord-channel-context.fake.yaml` as the resolver reference and fake fixture.
14. Use `docs/architecture/discord-topology-reconciliation.md` and `examples/discord-topology-reconciliation.fake.yaml` when validating category/channel discovery before provisioning.
15. Use `docs/architecture/discord-context-namespace-provisioning.md` and `examples/discord-context-provisioning.fake.yaml` when reviewing approved draft context artifacts.
