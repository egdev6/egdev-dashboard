...
Track the operator-facing Discord rehearsal in #132, the documentation/scaffolding clarification in #146, and the legacy-routing cleanup in #150 before creating any baseline tag or release note.
...
- managed channel routing based on persisted semantic metadata/IDs instead of display-name inference;
- global context, project/category context, skills/packs, memory/context separation, and approval-response tests;
...
1. **Manual Discord verification gate**: execute or explicitly defer #132 using the private Discord manual verification guide, with #146 documenting the current private control and Project Manager scaffolding expectations and #150 removing legacy routed-channel rehearsal requirements.
