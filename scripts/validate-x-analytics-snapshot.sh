...
grep -F "network_namespace_key: $X_NAMESPACE" "$FIXTURE_PATH" >/dev/null || fail "fixture does not declare expected X namespace"
...
validate_content_metric_records() {
  awk '
    ...
    function validate_record() {
      ...
      if (!planning || !ledger || !status || !impressions || !likes || !replies || !reposts || !quote_posts || !clicks || !engagement || !followers) {
    ...
    in_record && /^      likes:/ { likes = 1 }
    in_record && /^      replies:/ { replies = 1 }
    in_record && /^      reposts:/ { reposts = 1 }
    in_record && /^      quote_posts:/ { quote_posts = 1 }
